...
| `toolName` | Strict allowlisted tool name, otherwise `tool` |
| `toolSummary` | Tool name, filename basename, or command category only |
| `message` | Fixed safe activity text and integer token/error counts |
| `timestamp` | Unix seconds |

It never forwards prompts, assistant output, file contents, full paths, command text, command output, queries, URLs, secrets, transcript paths, or terminal focus URLs. Unknown tools are reduced to `tool`; shell commands are reduced to one of `build`, `filesystem`, `package-manager`, `process`, `test`, `version-control`, or `shell`.
